...
// import {version} from '../node_modules/react-d3-tree/package.json';
// import { collection, getDocs } from 'firebase/firestore';
// import { EmployeeList, List } from './components/emList';
...
import MixedNodeElement from '../../../src/components/MixedNodeElement';
import PureSvgNodeElement from '../../../src/components/PureSvgNodeElement';
import MixedNodeInputElement from '../../../src/components/MixedNodeInputElement';
import './App.css';
import { collection, collectionGroup, getDocs, CollectionReference, addDoc } from 'firebase/firestore';
...
const transformFirebaseData = (data) => {
  ...
  data.forEach((node) => {
    node.children = [];
    nodeMap.set(node.employeeID, node);
  });

  // Build the tree structure
  const treeData = [];
  data.forEach((node) => {
    if (node.managerID === null || node.managerID === undefined) {
      treeData.push(node);
    } else {
...
// Fetch data from Firebase
  ...
  class App extends React.Component {
    constructor() {
      ...
      this.fetchDataFromFirebase = async () => {

        // Assuming "employees" is the collection in Firestore
        ...
        this.transformedData = transformFirebaseData(combinedFirebaseData);
      ...
      this.state = {
        ...
        totalNodeCount: countNodes(0, Array.isArray(this.transformedData) ? this.transformedData[0] : this.transformedData),
        ...
        renderCustomNodeElement: this.customNodeFnMapping['svg'].fn,
        styles: {
          nodes: {
            node: {
              circle: {
    ...
    setTreeData(data) {
      this.setState({
        data,
        totalNodeCount: this.countNodes(0, Array.isArray(data) ? data[0] : data),
      });
    }
    ...
    handleCustomNodeFnChange = evt => {
      ...
      this.setState({ renderCustomNodeElement: this.customNodeFnMapping[customNodeKey].fn });
    };
    ...
    addChildNode = () => {
      const mydata = clone(this.state.data);
      const target = mydata[0].children ? mydata[0].children : mydata[0]._children;
              ...
              <div className="controls-container">
                <div className="prop-container">
                  ...
                  <div style={{ marginBottom: '5px' }}>
                    <button
                      ...
                      onClick={() => this.setTreeData(this.transformedData)}
                    >
                      Org chart (small)
                    </button>
                  </div>
                ...
                <div className="prop-container">
                  ...
                  <input
                    ...
                    onChange={evt => this.setScaleExtent({
                ...
                <div className="prop-container">
                  ...
                  <input
                    ...
                    onChange={evt => this.setSeparation({
